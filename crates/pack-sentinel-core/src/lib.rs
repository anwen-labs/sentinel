...
pub fn catalog() -> Vec<engine::RuleMeta> {
    ...
    vec![
        ...
        RuleMeta { id: "HOST-NETWORK-MODE", title: "Host network mode", target: t, severity: High, controls: &["CWE-668", "CIS-Docker-5.9", "ATTACK-T1611"], summary: "network_mode: host removes network namespace isolation; the container shares the host's network stack.", fix: "Use a user-defined bridge network and publish only the needed ports.", strict: false },
        RuleMeta { id: "WEAK-DEFAULT-CREDENTIAL", title: "Weak / default credential", target: t, severity: High, controls: &["CWE-798", "CWE-1392", "ATTACK-T1078.001"], summary: "A secret-like env var is set to a weak or default value (admin, password, changeme…).", fix: "Set a strong unique secret; inject via a secrets manager, not inline.", strict: false },
        RuleMeta { id: "DATABASE-AUTH-DISABLED", title: "Database auth disabled", target: t, severity: High, controls: &["CWE-287", "CWE-1392", "ATTACK-T1078"], summary: "An env var disables database authentication entirely (MYSQL_ALLOW_EMPTY_PASSWORD, POSTGRES_HOST_AUTH_METHOD=trust, …) — anyone who reaches the DB connects with no credentials.", fix: "Remove the empty-password/trust switch; set a strong password and restrict the DB to an internal network.", strict: false },
        RuleMeta { id: "REACHABLE-WEAK-CREDENTIAL", title: "Reachable weak-credential path", target: t, severity: High, controls: &["CWE-668", "CWE-1390", "ATTACK-T1078.001"], summary: "Cross-resource: an internet-reachable service (0.0.0.0 port) has a depends_on path to a service that uses a weak/default credential — an exploitable chain, not a single flag.", fix: "Don't expose the service directly; bind to 127.0.0.1 or use a gateway, and replace weak/default credentials.", strict: false },
        RuleMeta { id: "SENSITIVE-HOST-PATH-MOUNT", title: "Sensitive host path mounted", target: t, severity: High, controls: &["CWE-552", "CWE-668", "ATTACK-T1611"], summary: "A sensitive host directory (/, /etc, /proc, /sys, …) is bind-mounted into a container.", fix: "Remove the bind mount or scope it to a specific non-sensitive subdirectory (read-only).", strict: false },
        RuleMeta { id: "HOST-PID-NAMESPACE", title: "Host PID namespace", target: t, severity: High, controls: &["CWE-668", "CIS-Docker-5.15", "ATTACK-T1611"], summary: "pid: host shares the host PID namespace — the container can see and signal host processes.", fix: "Remove 'pid: host'; containers should use their own PID namespace.", strict: false },
        ...
        RuleMeta { id: "SECURITY-PROFILE-DISABLED", title: "Seccomp/AppArmor disabled", target: t, severity: High, controls: &["CWE-693", "CIS-Docker-5.1", "ATTACK-T1562.001"], summary: "security_opt sets seccomp:unconfined or apparmor:unconfined, removing a kernel-hardening layer that blocks dangerous syscalls.", fix: "Use the default seccomp/AppArmor profiles or a scoped custom one.", strict: false },
        RuleMeta { id: "HOST-USERNS-MODE", title: "Host user namespace", target: t, severity: High, controls: &["CWE-281", "CIS-Docker-5.30", "ATTACK-T1611"], summary: "userns_mode: host disables user-namespace remapping, so root in the container maps to root on the host.", fix: "Remove 'userns_mode: host' and enable user-namespace remapping.", strict: false },
        RuleMeta { id: "SECRET-IN-ENVIRONMENT", title: "Secret in environment", target: t, severity: Medium, controls: &["CWE-256", "CWE-798", "ATTACK-T1552.001"], summary: "A secret-like env var has an inline literal value — it ends up in version control and `docker inspect`.", fix: "Use Compose/Docker secrets or an external secrets manager; reference, don't inline.", strict: false },
        RuleMeta { id: "SENSITIVE-PORT-PUBLISHED-ALL-IFACES", title: "Datastore port on all interfaces", target: t, severity: Medium, controls: &["CWE-668", "ATTACK-T1190"], summary: "A database/cache/admin port is published on 0.0.0.0 — often reachable externally.", fix: "Bind to 127.0.0.1 or an internal network; do not publish datastores.", strict: false },
        ...
        RuleMeta { id: "PORT-PUBLISHED-ALL-IFACES", title: "Port on all interfaces", target: t, severity: Low, controls: &["CWE-668", "ATTACK-T1190"], summary: "A port is published on 0.0.0.0 — reachable from any interface.", fix: "Bind to a specific interface (e.g. 127.0.0.1) unless external access is intended.", strict: false },
        ...
        RuleMeta { id: "NO-RESOURCE-LIMITS", title: "No memory limit", target: t, severity: Low, controls: &["CWE-400", "ATTACK-T1499"], summary: "No memory limit is set — a runaway container can exhaust host memory (DoS).", fix: "Set a memory limit (deploy.resources.limits.memory, or mem_limit in v2).", strict: true },
    ]
}
