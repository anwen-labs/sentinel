...
pub fn catalog() -> Vec<engine::RuleMeta> {
    ...
    vec![
        RuleMeta { id: "DOCKER-SOCKET-MOUNT", title: "Docker socket mounted", target: t, severity: Critical, controls: &["CWE-250", "CIS-Docker-5.31", "ATTACK-T1611", "NIST-800-53-AC-6", "NIST-800-53-CM-7", "OWASP-Docker-D05"], summary: "A container bind-mounts the Docker socket (/var/run/docker.sock or the /run/docker.sock symlink), giving it full control of the Docker daemon — effectively root on the host.", fix: "Remove the mount; if daemon access is required, use a scoped socket proxy, read-only.", strict: false },
        RuleMeta { id: "REACHABLE-HOST-TAKEOVER", title: "Reachable host-takeover path", target: t, severity: Critical, controls: &["CWE-668", "CWE-250", "ATTACK-T1611", "NIST-800-53-SC-7"], summary: "Cross-resource: an internet-reachable service has a depends_on path to a service that mounts the Docker socket or runs privileged — reaching the front door chains to full host compromise.", fix: "Keep docker.sock/privileged off any internet-reachable path; don't expose the front-end directly.", strict: false },
        RuleMeta { id: "PRIVILEGED-CONTAINER", title: "Privileged container", target: t, severity: Critical, controls: &["CWE-250", "CIS-Docker-5.4", "ATTACK-T1611", "NIST-800-53-AC-6", "NIST-800-53-SC-39", "OWASP-Docker-D04"], summary: "privileged: true disables container isolation — near-equivalent to root on the host.", fix: "Drop 'privileged'; grant only the specific capabilities required.", strict: false },
        RuleMeta { id: "CAP-ADD-ALL", title: "All capabilities granted", target: t, severity: Critical, controls: &["CWE-250", "CIS-Docker-5.3", "ATTACK-T1611", "NIST-800-53-AC-6", "OWASP-Docker-D04"], summary: "cap_add: [ALL] grants every Linux capability — equivalent to privileged and trivially escapable to the host.", fix: "Remove cap_add: ALL; drop all caps and add back only the few needed.", strict: false },
        RuleMeta { id: "DANGEROUS-CAPABILITY", title: "Dangerous capability added", target: t, severity: High, controls: &["CWE-250", "CIS-Docker-5.3", "ATTACK-T1611", "OWASP-Docker-D04"], summary: "cap_add grants a high-risk Linux capability (e.g. SYS_ADMIN, NET_ADMIN) that can enable container escape.", fix: "Remove the capability; if required, justify and isolate the workload.", strict: false },
        RuleMeta { id: "HOST-NETWORK-MODE", title: "Host network mode", target: t, severity: High, controls: &["CWE-668", "CIS-Docker-5.9", "ATTACK-T1611", "NIST-800-53-SC-7", "OWASP-Docker-D03"], summary: "network_mode: host removes network namespace isolation; the container shares the host's network stack.", fix: "Use a user-defined bridge network and publish only the needed ports.", strict: false },
        RuleMeta { id: "WEAK-DEFAULT-CREDENTIAL", title: "Weak / default credential", target: t, severity: High, controls: &["CWE-798", "CWE-1392", "ATTACK-T1078.001", "NIST-800-53-IA-5", "NIST-800-53-IA-5(1)", "OWASP-Docker-D06"], summary: "A secret-like env var is set to a weak or default value (admin, password, changeme…).", fix: "Set a strong unique secret; inject via a secrets manager, not inline.", strict: false },
        RuleMeta { id: "DATABASE-AUTH-DISABLED", title: "Database auth disabled", target: t, severity: High, controls: &["CWE-287", "CWE-1392", "ATTACK-T1078", "NIST-800-53-IA-2", "OWASP-Docker-D06"], summary: "An env var disables database authentication entirely (MYSQL_ALLOW_EMPTY_PASSWORD, POSTGRES_HOST_AUTH_METHOD=trust, …) — anyone who reaches the DB connects with no credentials.", fix: "Remove the empty-password/trust switch; set a strong password and restrict the DB to an internal network.", strict: false },
        RuleMeta { id: "REACHABLE-WEAK-CREDENTIAL", title: "Reachable weak-credential path", target: t, severity: High, controls: &["CWE-668", "CWE-1390", "ATTACK-T1078.001", "NIST-800-53-IA-5", "NIST-800-53-IA-5(1)"], summary: "Cross-resource: an internet-reachable service (0.0.0.0 port) has a depends_on path to a service that uses a weak/default credential — an exploitable chain, not a single flag.", fix: "Don't expose the service directly; bind to 127.0.0.1 or use a gateway, and replace weak/default credentials.", strict: false },
        RuleMeta { id: "SENSITIVE-HOST-PATH-MOUNT", title: "Sensitive host path mounted", target: t, severity: High, controls: &["CWE-552", "CWE-668", "ATTACK-T1611", "NIST-800-53-SC-39", "OWASP-Docker-D05"], summary: "A sensitive host directory (/, /etc, /proc, /sys, …) is bind-mounted into a container.", fix: "Remove the bind mount or scope it to a specific non-sensitive subdirectory (read-only).", strict: false },
        RuleMeta { id: "HOST-PID-NAMESPACE", title: "Host PID namespace", target: t, severity: High, controls: &["CWE-668", "CIS-Docker-5.15", "ATTACK-T1611", "NIST-800-53-SC-39", "OWASP-Docker-D05"], summary: "pid: host shares the host PID namespace — the container can see and signal host processes.", fix: "Remove 'pid: host'; containers should use their own PID namespace.", strict: false },
        RuleMeta { id: "HOST-IPC-NAMESPACE", title: "Host IPC namespace", target: t, severity: High, controls: &["CWE-668", "CIS-Docker-5.16", "ATTACK-T1611", "OWASP-Docker-D05"], summary: "ipc: host shares the host IPC namespace — breaks process isolation.", fix: "Remove 'ipc: host'; containers should use their own IPC namespace.", strict: false },
        RuleMeta { id: "SECURITY-PROFILE-DISABLED", title: "Seccomp/AppArmor disabled", target: t, severity: High, controls: &["CWE-693", "CIS-Docker-5.1", "ATTACK-T1562.001", "NIST-800-53-CM-7", "OWASP-Docker-D04"], summary: "security_opt sets seccomp:unconfined or apparmor:unconfined, removing a kernel-hardening layer that blocks dangerous syscalls.", fix: "Use the default seccomp/AppArmor profiles or a scoped custom one.", strict: false },
        RuleMeta { id: "HOST-USERNS-MODE", title: "Host user namespace", target: t, severity: High, controls: &["CWE-281", "CIS-Docker-5.30", "ATTACK-T1611", "NIST-800-53-SC-39", "NIST-800-53-AC-6", "OWASP-Docker-D04"], summary: "userns_mode: host disables user-namespace remapping, so root in the container maps to root on the host.", fix: "Remove 'userns_mode: host' and enable user-namespace remapping.", strict: false },
        RuleMeta { id: "SECRET-IN-ENVIRONMENT", title: "Secret in environment", target: t, severity: Medium, controls: &["CWE-256", "CWE-798", "ATTACK-T1552.001", "NIST-800-53-IA-5(7)", "OWASP-Docker-D06"], summary: "A secret-like env var has an inline literal value — it ends up in version control and `docker inspect`.", fix: "Use Compose/Docker secrets or an external secrets manager; reference, don't inline.", strict: false },
        RuleMeta { id: "SENSITIVE-PORT-PUBLISHED-ALL-IFACES", title: "Datastore port on all interfaces", target: t, severity: Medium, controls: &["CWE-668", "ATTACK-T1190", "NIST-800-53-SC-7", "NIST-800-53-SC-7(5)", "OWASP-Docker-D03"], summary: "A database/cache/admin port is published on 0.0.0.0 — often reachable externally.", fix: "Bind to 127.0.0.1 or an internal network; do not publish datastores.", strict: false },
        RuleMeta { id: "IMAGE-UNPINNED", title: "Image not pinned by digest", target: t, severity: Low, controls: &["CWE-494", "CWE-1357", "ATTACK-T1195.002", "OWASP-Docker-D08"], summary: "An image uses a tag (or :latest) rather than a digest — what runs can change silently.", fix: "Pin by digest (image@sha256:…).", strict: false },
        RuleMeta { id: "CONTAINER-RUNS-AS-ROOT-OR-UNKNOWN", title: "Runs as root / unknown user", target: t, severity: Low, controls: &["CWE-250", "CIS-Docker-4.1", "OWASP-Docker-D01"], summary: "The service runs as root, or no user is declared so it can't be confirmed non-root.", fix: "Set a non-root 'user:'; declare it explicitly even if the base image sets one.", strict: false },
        RuleMeta { id: "WRITABLE-ROOT-FILESYSTEM", title: "Writable root filesystem", target: t, severity: Low, controls: &["CWE-732", "CIS-Docker-5.12", "NIST-800-53-AC-6"], summary: "read_only is not set, so an attacker can persist tooling in the container filesystem.", fix: "Set 'read_only: true' and mount specific writable paths as tmpfs/volumes.", strict: false },
        RuleMeta { id: "PORT-PUBLISHED-ALL-IFACES", title: "Port on all interfaces", target: t, severity: Low, controls: &["CWE-668", "ATTACK-T1190", "NIST-800-53-CM-7", "OWASP-Docker-D03"], summary: "A port is published on 0.0.0.0 — reachable from any interface.", fix: "Bind to a specific interface (e.g. 127.0.0.1) unless external access is intended.", strict: false },
        RuleMeta { id: "NO-NEW-PRIVILEGES-MISSING", title: "no-new-privileges not set", target: t, severity: Low, controls: &["CWE-250", "ATTACK-T1548", "OWASP-Docker-D04"], summary: "Processes can gain privileges via setuid binaries because no-new-privileges isn't set.", fix: "Add 'security_opt: [\"no-new-privileges:true\"]'.", strict: true },
        RuleMeta { id: "CAP-DROP-ALL-MISSING", title: "Capabilities not dropped", target: t, severity: Low, controls: &["CWE-250", "OWASP-Docker-D05"], summary: "The service keeps Docker's default capability set instead of dropping all and adding back only what's needed.", fix: "Add 'cap_drop: [ALL]' and 'cap_add' only the capabilities you need.", strict: true },
        RuleMeta { id: "NO-RESOURCE-LIMITS", title: "No memory limit", target: t, severity: Low, controls: &["CWE-400", "ATTACK-T1499", "NIST-800-53-SC-6", "OWASP-Docker-D07"], summary: "No memory limit is set — a runaway container can exhaust host memory (DoS).", fix: "Set a memory limit (deploy.resources.limits.memory, or mem_limit in v2).", strict: true },
    ]
}
