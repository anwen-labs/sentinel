...
pub fn catalog() -> Vec<engine::RuleMeta> {
    ...
    vec![
        ...
        RuleMeta { id: "SENSITIVE-HOST-PATH-MOUNT", title: "Sensitive host path mounted", target: t, severity: High, controls: &["CWE-552", "CWE-668", "ATTACK-T1611", "NIST-800-53-SC-39"], summary: "A sensitive host directory (/, /etc, /proc, /sys, …) is bind-mounted into a container.", fix: "Remove the bind mount or scope it to a specific non-sensitive subdirectory (read-only).", strict: false },
        RuleMeta { id: "HOST-PID-NAMESPACE", title: "Host PID namespace", target: t, severity: High, controls: &["CWE-668", "CIS-Docker-5.15", "ATTACK-T1611", "NIST-800-53-SC-39"], summary: "pid: host shares the host PID namespace — the container can see and signal host processes.", fix: "Remove 'pid: host'; containers should use their own PID namespace.", strict: false },
        RuleMeta { id: "HOST-IPC-NAMESPACE", title: "Host IPC namespace", target: t, severity: High, controls: &["CWE-668", "CIS-Docker-5.16", "ATTACK-T1611"], summary: "ipc: host shares the host IPC namespace — breaks process isolation.", fix: "Remove 'ipc: host'; containers should use their own IPC namespace.", strict: false },
        RuleMeta { id: "SECURITY-PROFILE-DISABLED", title: "Seccomp/AppArmor disabled", target: t, severity: High, controls: &["CWE-693", "CIS-Docker-5.1", "ATTACK-T1562.001", "NIST-800-53-CM-7"], summary: "security_opt sets seccomp:unconfined or apparmor:unconfined, removing a kernel-hardening layer that blocks dangerous syscalls.", fix: "Use the default seccomp/AppArmor profiles or a scoped custom one.", strict: false },
        RuleMeta { id: "HOST-USERNS-MODE", title: "Host user namespace", target: t, severity: High, controls: &["CWE-281", "CIS-Docker-5.30", "ATTACK-T1611", "NIST-800-53-SC-39", "NIST-800-53-AC-6"], summary: "userns_mode: host disables user-namespace remapping, so root in the container maps to root on the host.", fix: "Remove 'userns_mode: host' and enable user-namespace remapping.", strict: false },
        RuleMeta { id: "SECRET-IN-ENVIRONMENT", title: "Secret in environment", target: t, severity: Medium, controls: &["CWE-256", "CWE-798", "ATTACK-T1552.001", "NIST-800-53-IA-5(7)"], summary: "A secret-like env var has an inline literal value — it ends up in version control and `docker inspect`.", fix: "Use Compose/Docker secrets or an external secrets manager; reference, don't inline.", strict: false },
        RuleMeta { id: "SENSITIVE-PORT-PUBLISHED-ALL-IFACES", title: "Datastore port on all interfaces", target: t, severity: Medium, controls: &["CWE-668", "ATTACK-T1190", "NIST-800-53-SC-7", "NIST-800-53-SC-7(5)"], summary: "A database/cache/admin port is published on 0.0.0.0 — often reachable externally.", fix: "Bind to 127.0.0.1 or an internal network; do not publish datastores.", strict: false },
        RuleMeta { id: "IMAGE-UNPINNED", title: "Image not pinned by digest", target: t, severity: Low, controls: &["CWE-494", "CWE-1357", "ATTACK-T1195.002"], summary: "An image uses a tag (or :latest) rather than a digest — what runs can change silently.", fix: "Pin by digest (image@sha256:…).", strict: false },
        ...
        RuleMeta { id: "PORT-PUBLISHED-ALL-IFACES", title: "Port on all interfaces", target: t, severity: Low, controls: &["CWE-668", "ATTACK-T1190", "NIST-800-53-CM-7"], summary: "A port is published on 0.0.0.0 — reachable from any interface.", fix: "Bind to a specific interface (e.g. 127.0.0.1) unless external access is intended.", strict: false },
        RuleMeta { id: "NO-NEW-PRIVILEGES-MISSING", title: "no-new-privileges not set", target: t, severity: Low, controls: &["CWE-250", "ATTACK-T1548"], summary: "Processes can gain privileges via setuid binaries because no-new-privileges isn't set.", fix: "Add 'security_opt: [\"no-new-privileges:true\"]'.", strict: true },
        ...
        RuleMeta { id: "NO-RESOURCE-LIMITS", title: "No memory limit", target: t, severity: Low, controls: &["CWE-400", "ATTACK-T1499", "NIST-800-53-SC-6"], summary: "No memory limit is set — a runaway container can exhaust host memory (DoS).", fix: "Set a memory limit (deploy.resources.limits.memory, or mem_limit in v2).", strict: true },
    ]
}
