...
pub fn catalog() -> Vec<engine::RuleMeta> {
    ...
    vec![
        ...
        RuleMeta { id: "K8S-HOST-NETWORK", title: "Host network namespace", target: t, severity: High, controls: &["CWE-668", "CIS-K8s-5.2.5", "ATTACK-T1611"], summary: "hostNetwork: true shares the node's network stack and bypasses NetworkPolicies.", fix: "Remove hostNetwork; use a Service.", strict: false },
        RuleMeta { id: "K8S-HOST-PID", title: "Host PID namespace", target: t, severity: High, controls: &["CWE-668", "CIS-K8s-5.2.3", "ATTACK-T1611"], summary: "hostPID: true lets the pod see and signal processes on the node.", fix: "Remove hostPID.", strict: false },
        RuleMeta { id: "K8S-HOST-IPC", title: "Host IPC namespace", target: t, severity: High, controls: &["CWE-668", "CIS-K8s-5.2.4", "ATTACK-T1611"], summary: "hostIPC: true shares the node's IPC namespace.", fix: "Remove hostIPC.", strict: false },
        RuleMeta { id: "K8S-HOSTPATH-MOUNT", title: "hostPath volume", target: t, severity: High, controls: &["CWE-552", "CIS-K8s-5.2.12", "ATTACK-T1611"], summary: "A hostPath volume mounts a node directory into the pod, escaping isolation (Critical for the Docker socket / sensitive paths).", fix: "Use a PersistentVolume/configMap/emptyDir; if unavoidable, mount a specific non-sensitive subdir read-only.", strict: false },
        RuleMeta { id: "K8S-DANGEROUS-CAPABILITY", title: "Dangerous capability added", target: t, severity: High, controls: &["CWE-250", "CIS-K8s-5.2.9", "ATTACK-T1611"], summary: "capabilities.add includes a high-risk capability (SYS_ADMIN, NET_ADMIN, …) enabling escape or host tampering.", fix: "Remove the capability; if required, justify and isolate.", strict: false },
        RuleMeta { id: "K8S-SECCOMP-UNCONFINED", title: "Seccomp unconfined", target: t, severity: High, controls: &["CWE-693", "CIS-K8s-5.7.2", "ATTACK-T1562.001"], summary: "seccompProfile type Unconfined removes the syscall filter that blocks dangerous kernel calls.", fix: "Use seccompProfile RuntimeDefault (or a scoped Localhost profile).", strict: false },
        RuleMeta { id: "K8S-RBAC-WILDCARD", title: "Wildcard RBAC permissions", target: t, severity: High, controls: &["CWE-269", "CIS-K8s-5.1.3", "ATTACK-T1098"], summary: "A Role/ClusterRole grants all verbs on all resources (*/*) — unrestricted within scope (Critical at cluster scope).", fix: "Scope rules to the specific apiGroups/resources/verbs needed.", strict: false },
        RuleMeta { id: "K8S-RBAC-SECRET-READ", title: "Broad Secret read access", target: t, severity: Medium, controls: &["CWE-522", "CIS-K8s-5.1.2", "ATTACK-T1552"], summary: "A Role/ClusterRole can get/list/watch Secrets — exposes credentials if the bound identity is compromised (High at cluster scope).", fix: "Scope to named secrets or use an external secrets store.", strict: false },
        RuleMeta { id: "K8S-ALLOW-PRIVILEGE-ESCALATION", title: "Privilege escalation allowed", target: t, severity: Medium, controls: &["CWE-250", "CIS-K8s-5.2.6", "ATTACK-T1548"], summary: "Container sets allowPrivilegeEscalation: true — a process can gain more privileges than its parent.", fix: "Set allowPrivilegeEscalation: false.", strict: false },
        RuleMeta { id: "K8S-SECRET-IN-MANIFEST", title: "Secret embedded in manifest", target: t, severity: Medium, controls: &["CWE-312", "CWE-798", "ATTACK-T1552.001"], summary: "A Secret embeds its data inline (base64 is not encryption) — it lands in version control and CI logs.", fix: "Use sealed-secrets/SOPS, an external secrets operator, or a cloud secret store.", strict: false },
