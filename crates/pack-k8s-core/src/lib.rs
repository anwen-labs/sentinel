...
pub fn catalog() -> Vec<engine::RuleMeta> {
    ...
    let t = "Kubernetes";
    vec![
        RuleMeta { id: "K8S-REACHABLE-NODE-COMPROMISE", title: "Reachable node-compromise path", target: t, severity: Critical, controls: &["CWE-668", "CWE-250", "ATTACK-T1611", "NIST-800-53-SC-7", "OWASP-K8s-K01"], summary: "Cross-resource: an external Service (NodePort/LoadBalancer, or a ClusterIP an Ingress or Gateway-API route points to) selects a Workload that runs privileged / adds a dangerous capability / mounts a sensitive hostPath — reaching the service chains to node or cluster takeover.", fix: "Keep node-takeover surfaces off anything an external Service selects; front with a hardened gateway.", strict: false },
        RuleMeta { id: "K8S-PRIVILEGED-CONTAINER", title: "Privileged container", target: t, severity: Critical, controls: &["CWE-250", "CIS-K8s-5.2.2", "ATTACK-T1611", "NIST-800-53-AC-6", "NIST-800-53-SC-39", "OWASP-K8s-K01"], summary: "A container sets securityContext.privileged: true — full host device/kernel access, trivial node takeover.", fix: "Remove privileged; grant only the specific capabilities required.", strict: false },
        RuleMeta { id: "K8S-CAP-ADD-ALL", title: "All capabilities added", target: t, severity: Critical, controls: &["CWE-250", "CIS-K8s-5.2.9", "ATTACK-T1611", "NIST-800-53-AC-6(1)", "OWASP-K8s-K01"], summary: "A container adds ALL Linux capabilities — equivalent to privileged.", fix: "Drop all capabilities and add back only the few needed.", strict: false },
        RuleMeta { id: "K8S-CLUSTER-ADMIN-BINDING", title: "cluster-admin granted", target: t, severity: Critical, controls: &["CWE-269", "CIS-K8s-5.1.1", "ATTACK-T1098", "NIST-800-53-AC-6", "OWASP-K8s-K02"], summary: "A (Cluster)RoleBinding binds the built-in cluster-admin role — full control of the cluster.", fix: "Bind a least-privilege role; reserve cluster-admin for break-glass.", strict: false },
        RuleMeta { id: "K8S-HOST-NETWORK", title: "Host network namespace", target: t, severity: High, controls: &["CWE-668", "CIS-K8s-5.2.5", "ATTACK-T1611", "NIST-800-53-SC-7", "OWASP-K8s-K01"], summary: "hostNetwork: true shares the node's network stack and bypasses NetworkPolicies.", fix: "Remove hostNetwork; use a Service.", strict: false },
        RuleMeta { id: "K8S-HOST-PID", title: "Host PID namespace", target: t, severity: High, controls: &["CWE-668", "CIS-K8s-5.2.3", "ATTACK-T1611", "NIST-800-53-SC-39", "OWASP-K8s-K01"], summary: "hostPID: true lets the pod see and signal processes on the node.", fix: "Remove hostPID.", strict: false },
        RuleMeta { id: "K8S-HOST-IPC", title: "Host IPC namespace", target: t, severity: High, controls: &["CWE-668", "CIS-K8s-5.2.4", "ATTACK-T1611", "NIST-800-53-SC-39", "OWASP-K8s-K01"], summary: "hostIPC: true shares the node's IPC namespace.", fix: "Remove hostIPC.", strict: false },
        RuleMeta { id: "K8S-HOSTPATH-MOUNT", title: "hostPath volume", target: t, severity: High, controls: &["CWE-552", "CIS-K8s-5.2.12", "ATTACK-T1611", "NIST-800-53-SC-39", "OWASP-K8s-K01"], summary: "A hostPath volume mounts a node directory into the pod, escaping isolation (Critical for the Docker socket / sensitive paths).", fix: "Use a PersistentVolume/configMap/emptyDir; if unavoidable, mount a specific non-sensitive subdir read-only.", strict: false },
        RuleMeta { id: "K8S-DANGEROUS-CAPABILITY", title: "Dangerous capability added", target: t, severity: High, controls: &["CWE-250", "CIS-K8s-5.2.9", "ATTACK-T1611", "NIST-800-53-AC-6", "OWASP-K8s-K01"], summary: "capabilities.add includes a high-risk capability (SYS_ADMIN, NET_ADMIN, …) enabling escape or host tampering.", fix: "Remove the capability; if required, justify and isolate.", strict: false },
        RuleMeta { id: "K8S-SECCOMP-UNCONFINED", title: "Seccomp unconfined", target: t, severity: High, controls: &["CWE-693", "CIS-K8s-5.7.2", "ATTACK-T1562.001", "NIST-800-53-CM-7", "OWASP-K8s-K01"], summary: "seccompProfile type Unconfined removes the syscall filter that blocks dangerous kernel calls.", fix: "Use seccompProfile RuntimeDefault (or a scoped Localhost profile).", strict: false },
        RuleMeta { id: "K8S-RBAC-WILDCARD", title: "Wildcard RBAC permissions", target: t, severity: High, controls: &["CWE-269", "CIS-K8s-5.1.3", "ATTACK-T1098", "NIST-800-53-AC-6", "OWASP-K8s-K02"], summary: "A Role/ClusterRole grants all verbs on all resources (*/*) — unrestricted within scope (Critical at cluster scope).", fix: "Scope rules to the specific apiGroups/resources/verbs needed.", strict: false },
        RuleMeta { id: "K8S-RBAC-SECRET-READ", title: "Broad Secret read access", target: t, severity: Medium, controls: &["CWE-522", "CIS-K8s-5.1.2", "ATTACK-T1552", "NIST-800-53-AC-6", "OWASP-K8s-K02"], summary: "A Role/ClusterRole can get/list/watch Secrets — exposes credentials if the bound identity is compromised (High at cluster scope).", fix: "Scope to named secrets or use an external secrets store.", strict: false },
        RuleMeta { id: "K8S-ALLOW-PRIVILEGE-ESCALATION", title: "Privilege escalation allowed", target: t, severity: Medium, controls: &["CWE-250", "CIS-K8s-5.2.6", "ATTACK-T1548", "NIST-800-53-AC-6", "OWASP-K8s-K01"], summary: "Container sets allowPrivilegeEscalation: true — a process can gain more privileges than its parent.", fix: "Set allowPrivilegeEscalation: false.", strict: false },
        RuleMeta { id: "K8S-SECRET-IN-MANIFEST", title: "Secret embedded in manifest", target: t, severity: Medium, controls: &["CWE-312", "CWE-798", "ATTACK-T1552.001", "NIST-800-53-IA-5(7)", "OWASP-K8s-K03"], summary: "A Secret embeds its data inline (base64 is not encryption) — it lands in version control and CI logs.", fix: "Use sealed-secrets/SOPS, an external secrets operator, or a cloud secret store.", strict: false },
        RuleMeta { id: "K8S-IMAGE-UNPINNED", title: "Image not pinned by digest", target: t, severity: Low, controls: &["CWE-494", "CWE-1357", "ATTACK-T1195.002"], summary: "A container image uses a tag rather than a digest — what runs can change silently.", fix: "Pin images by digest (repo@sha256:…).", strict: false },
        RuleMeta { id: "K8S-CONTAINER-RUNS-AS-ROOT", title: "Not provably non-root", target: t, severity: Low, controls: &["CWE-250", "CIS-K8s-5.2.7", "OWASP-K8s-K01"], summary: "No runAsNonRoot: true and no non-zero runAsUser, so the container can't be confirmed non-root.", fix: "Set runAsNonRoot: true and a non-zero runAsUser.", strict: false },
        RuleMeta { id: "K8S-READONLY-ROOTFS-MISSING", title: "Writable root filesystem", target: t, severity: Low, controls: &["CWE-732", "OWASP-K8s-K01"], summary: "readOnlyRootFilesystem is not set, so an attacker can persist tooling in the container filesystem.", fix: "Set readOnlyRootFilesystem: true and mount writable paths explicitly.", strict: true },
        RuleMeta { id: "K8S-ALLOW-PRIV-ESC-NOT-DISABLED", title: "Priv-escalation not disabled", target: t, severity: Low, controls: &["CWE-250", "ATTACK-T1548", "OWASP-K8s-K01"], summary: "allowPrivilegeEscalation is not explicitly false (defaults to true).", fix: "Set allowPrivilegeEscalation: false.", strict: true },
        RuleMeta { id: "K8S-AUTOMOUNT-SA-TOKEN", title: "SA token auto-mounted", target: t, severity: Low, controls: &["CWE-668", "CIS-K8s-5.1.6", "ATTACK-T1552.001", "NIST-800-53-AC-6", "OWASP-K8s-K03"], summary: "automountServiceAccountToken is not disabled, so the API token is mounted into every pod.", fix: "Set automountServiceAccountToken: false unless the workload calls the Kubernetes API.", strict: true },
    ]
}
