...
pub fn catalog() -> Vec<engine::RuleMeta> {
    ...
    vec![
        RuleMeta { id: "GHA-PWN-REQUEST", title: "pwn-request (untrusted checkout)", target: t, severity: Critical, controls: &["CWE-94", "CWE-829", "ATTACK-T1195.002", "NIST-800-53-AC-6"], summary: "Cross-resource: a privileged untrusted trigger (pull_request_target / workflow_run) plus a step that checks out the attacker-controlled PR ref — fork code runs with the repo's write token and secrets.", fix: "Don't run untrusted PR code under pull_request_target; use pull_request, or split safe build from privileged job.", strict: false },
        RuleMeta { id: "GHA-SCRIPT-INJECTION", title: "Script injection in run:", target: t, severity: High, controls: &["CWE-94", "CWE-78", "ATTACK-T1059", "NIST-800-53-SI-10"], summary: "A run: step interpolates attacker-controlled ${{ github.event.* }} (issue/PR title/body, comment, head_ref, …) straight into the shell — command injection.", fix: "Pass the value via env: and reference \"$VAR\" quoted; never interpolate github.event.* into run:.", strict: false },
        RuleMeta { id: "GHA-BROAD-PERMISSIONS", title: "Over-broad token permissions", target: t, severity: Medium, controls: &["CWE-272", "CWE-250", "ATTACK-T1195.002", "NIST-800-53-AC-6"], summary: "permissions: write-all grants the GITHUB_TOKEN write to every scope, so any compromised step can tamper widely.", fix: "Default to read-only permissions and grant only the specific write scopes a job needs.", strict: false },
        ...
        RuleMeta { id: "GHA-SECRETS-INHERIT", title: "secrets: inherit", target: t, severity: Medium, controls: &["CWE-200", "CWE-668", "ATTACK-T1552", "NIST-800-53-AC-6"], summary: "A job calls a reusable workflow with secrets: inherit, passing ALL caller secrets rather than only those needed.", fix: "Pass only the specific secrets the reusable workflow needs.", strict: false },
        RuleMeta { id: "GHA-UNPINNED-ACTION", title: "Action not pinned to SHA", target: t, severity: Low, controls: &["CWE-494", "CWE-829", "ATTACK-T1195.002", "NIST-800-53-SR-11"], summary: "A third-party action is pinned to a mutable tag/branch instead of a commit SHA — whoever controls that ref can change the code your workflow runs.", fix: "Pin actions to a full commit SHA; track updates with Dependabot.", strict: false },
    ]
}
