...
pub fn catalog() -> Vec<engine::RuleMeta> {
    ...
    vec![
        ...
        RuleMeta { id: "DOCKERFILE-TLS-VERIFICATION-DISABLED", title: "TLS verification disabled", target: t, severity: High, controls: &["CWE-295", "ATTACK-T1195.002", "NIST-800-53-SC-23"], summary: "A RUN downloads with curl -k / wget --no-check-certificate, disabling TLS certificate verification — the payload can be swapped in transit.", fix: "Remove the insecure flag; fix CA trust and verify a checksum/signature of the download.", strict: false },
        ...
        RuleMeta { id: "DOCKERFILE-ROOT-USER", title: "Runs as root", target: t, severity: Medium, controls: &["CWE-250", "CIS-Docker-4.1", "NIST-800-53-AC-6"], summary: "The image sets USER root or never sets a USER, so it defaults to root.", fix: "Add a non-root USER instruction (and create the user) before the entrypoint.", strict: false },
        RuleMeta { id: "DOCKERFILE-ADD-REMOTE-URL", title: "ADD fetches a remote URL", target: t, severity: Medium, controls: &["CWE-494", "ATTACK-T1195.002", "NIST-800-53-SI-7"], summary: "ADD fetches a remote URL with no integrity check (and auto-extracts archives).", fix: "Use COPY for local files, or RUN curl with a checksum verification step.", strict: false },
        RuleMeta { id: "DOCKERFILE-BUILD-SECRET", title: "Secret baked into the image", target: t, severity: Medium, controls: &["CWE-798", "ATTACK-T1552.001", "NIST-800-53-IA-5(7)"], summary: "A secret-like ENV/ARG has an inline value — it is baked into image layers.", fix: "Use BuildKit secrets (RUN --mount=type=secret) or runtime env, not ENV/ARG.", strict: false },
        RuleMeta { id: "DOCKERFILE-BASE-IMAGE-UNPINNED", title: "Base image not pinned", target: t, severity: Low, controls: &["CWE-494", "CWE-1357", "ATTACK-T1195.002", "NIST-800-53-SI-7"], summary: "The base image is not pinned by digest — the build is not reproducible.", fix: "Pin the base image by digest: FROM repo@sha256:…", strict: false },
